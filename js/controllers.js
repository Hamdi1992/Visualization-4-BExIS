    ...
    $scope.parse = text => {
      ...
      try {
        ...
        $timeout(function() {
          ...
          $scope.chart = $scope.charts.filter(d => {return d.title() == 'Box plot'})[0];
          $scope.model = $scope.chart ? $scope.chart.model() : null;

          // adding auto selection ** Hamdi
          var chart = raw.charts.values()[4];
          var chart = raw.charts.values().filter(d => {return d.title() == 'Voronoi Tessellation'})[0];
          $scope.chart = chart;
          //console.log(Object.keys( $scope.chart ));

          
          $scope.model = $scope.chart.model();
          //console.log(Object.keys( $scope.model ));

          var metadata = parser.metadata(text); // Dictionary Name- type data
          //console.log( metadata);

          var dimensions  =  $scope.model.dimensions();
          //console.log(Object.keys( $scope.model.dimensions()));
          //console.log(typeof( $scope.model.dimensions()));
          //console.log( $scope.model.dimensions());

          //Here we should add the mapping of the dimensions to the metadata
